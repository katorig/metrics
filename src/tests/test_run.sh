...
kinit -kt ${LOGIN}.keytab ${LOGIN}@CORP.TELE2.RU

#cd /opt/conda/envs/app-env/lib/python3.7/site-packages/tele2_metrics/
cd ../tele2_metrics
chmod +x tele2_metrics_tool.py
./tele2_metrics_tool.py --help
./tele2_metrics_tool.py compare_new_df_with_retro --notify 1 --model_id 135 --stage_db 'hadoop' \
...
if [ $? -eq 0 ]; then
  ./tele2_metrics_tool.py check_df_for_duplicates --notify 1 --model_id 135 --stage_db 'hadoop' \
...
./tele2_metrics_tool.py check_if_data_in_table --model_id 135 --final_db 'teradata' \
